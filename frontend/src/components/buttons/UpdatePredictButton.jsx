...
const UpdatePredictButton = props => {
    ...
    // todo:
    const {mutate, isLoading, error, isSuccess} = useMutation(() => dataProvider.updatePredict(resource, request_data), {
        onError: (error) => {
    ...
    return (
        <Tooltip title="Update predict" placement='top'>
            <IconButton onClick={handleCallPredict} sx={{width: "5%"}} disabled={isLoading} {...props}>
