...
export const clickOnMobileMenu = strCurrentClasses => {
  ...
  if (currentClasses.includes('main-hidden')) {
    nextClasses = currentClasses.filter(x => x !== 'main-hidden').join(' ');
  } else {
    nextClasses = `${currentClasses.join(' ')} main-hidden`;
  }
  return {
